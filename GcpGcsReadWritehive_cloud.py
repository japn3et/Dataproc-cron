...
def main():
   ...
   gcs_df = spark.read.csv("[file_location]",mode='dropmalformed',schema=custstructtype1)
   gcs_df.show(10)
   ...
   print(curts)
   gcs_df.repartition(2).write.json("[file_location]"+curts)
   ...
   curts = spark.createDataFrame([1], IntegerType()).withColumn("curts", current_timestamp()).select(date_format(col("curts"), "yyyyMMddHHmmSS")).first()[0]
   print(curts)
   gcs_df.repartition(2).write.mode("overwrite").csv("[bucket]/cust_csv")
   print("gcs Write Completed Successfully")
